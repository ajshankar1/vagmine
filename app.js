...
/* ═══════════════════════════════════════════
   INTERACTIVE GLOBE — Canvas Animation
   Shows India → Korea, China, Sri Lanka, USA
═══════════════════════════════════════════ */
function initGlobe() {
  ...
  /* Export country coordinates [lat, lon] */
  const countries = [
    { name: 'India (HQ)', lat: 8.9, lon: 77.7, home: true },   // Tirunelveli
  ...
  function drawGlobe(rot) {
    ...
    sphereGrd.addColorStop(0, 'rgba(30,30,32,0.9)');
    sphereGrd.addColorStop(1, 'rgba(14,14,16,0.95)');
    ctx.beginPath();
    ...
    for (let lat = -60; lat <= 60; lat += 30) {
      ...
      ctx.beginPath();
      ctx.ellipse(cx, cy + y, r, r * 0.15, 0, 0, Math.PI * 2);
    ...
    for (let lon = 0; lon < 180; lon += 30) {
      const theta = (lon + rot) * Math.PI / 180;
      ctx.beginPath();
      ctx.ellipse(cx, cy, R, R * Math.abs(Math.cos(theta)), theta, 0, Math.PI * 2);
    ...
    /* Draw arc connections from India to each country */
    ...
    countries.slice(1).forEach(c => {
      ...
      if (hPos.z > 0 && cPos.z > 0) {
        const hp = project(hPos.x, hPos.y);
        const cp = project(cPos.x, cPos.y);
        const mx = (hp.px + cp.px) / 2;
        const my = (hp.py + cp.py) / 2 - 50;
        ctx.beginPath();
        ctx.moveTo(hp.px, hp.py);
        ctx.quadraticCurveTo(mx, my, cp.px, cp.py);
    ...
    /* Sphere overlay / shine */
    const shineGrd = ctx.createRadialGradient(cx - R * 0.35, cy - R * 0.35, 0, cx, cy, R);
    shineGrd.addColorStop(0, 'rgba(255,255,255,0.05)');
    shineGrd.addColorStop(0.5, 'rgba(255,255,255,0)');
...
Company Details:
- Founded: 2010 | Third-generation management | 40+ years combined expertise
- Production: 2,400+ MT/year
- Address: B-59, SIPCOT Industrial Growth Centre, Gangaikondan, Tirunelveli-627352, Tamil Nadu, India
- USA Office: Delaware, USA (North American subsidiary)
- Phone & WhatsApp: +91 9313146672
- Email: info@vagmineoverseas.com
- Certifications: ISO 9001:2015, BIS Certified, MSME Registered
- Export Markets: India, South Korea, China, Sri Lanka, USA, UAE
- Key Clients: Sundaram Industries (TVS Group), Yokohama-ATG, BKT Tyres, Nexen Tyre, Global Rubber Pvt Ltd (Sri Lanka)

Products:
1. FRRC - Fiber Reinforced Rubber Compound: ½–1 inch granules, quick mill dispersion, excellent tack for tire building, ideal for solid tyres
2. FRC - Friction Rubber Compound (Baled): Enhanced durability and friction for demanding industrial and off-highway applications
3. FRC-25: Grade 25, medium hardness, commercial vehicle braking applications
4. FRC-10/12: Grades 10 & 12, lower durometer, lighter duty friction, available slab or granule form
5. Chopped Friction Cord: High-tenacity cord chopped to spec, reinforcement filler for brake/clutch linings
6. Granulated Friction Rubber (Long-Sheeted Rolls): 30–45ft × 39in × 7mm sheets, custom preforms, ready for solid tyre building
7. Reprocessed Rubber Compound: Cost-effective general industrial rubber compound
8. Tread Rubber Compounds: Abrasion resistant, high flexibility, superior tear strength for long service life
9. Industrial Hydraulic Seals: High-pressure, oil-resistant, custom profiles available
10. Pneumatic Rubber Seals: Low friction, air cylinders and actuators, long service life

Your Role:
- Answer product queries professionally and helpfully
- Help buyers identify the right product (FRC grade, format, application)
- Collect lead info after 3 messages (name, company, phone/email, requirement)
- Guide toward requesting quote or sample
- Be concise (3-5 sentences per reply), professional, action-oriented

Always end with a helpful next step — quote request, sample, WhatsApp, or calling.`;
...
function quickReply(text) {
  document.getElementById('aiQuick')?.remove();
  addUserMsg(text);
  getAIReply(text);
}
...
async function getAIReply(text) {
  ...
  try {
    ...
    const reply = data.reply || 'Please contact us at +91 9313146672 or info@vagmineoverseas.com';
    aiMsgs.push({ role: 'assistant', content: reply });
    addBotMsg(reply);
    if (aiMsgs.filter(m => m.role === 'user').length === 3) {
      setTimeout(() => {
        const nudge = 'To arrange a formal quote or callback, could you share your **name, company, and contact number**? Our team responds within 2 hours. 😊';
    ...
    document.getElementById('aiTyping')?.remove();
    addBotMsg('Having trouble connecting. Please reach us directly:\n📞 **+91 93131 46672**\n✉ **info@vagmineoverseas.com**');
  } finally {
    aiTyping = false;
